...
**T8-R06 实现证据（后续菜单调整）**：`CategoryDisplaySettingsSection.svelte` 已移动到“高级与视觉”独立二级菜单中，提供一级/二级字号与图标滑块；Home/Sidebar/CategorySection/Search/Preview 通过 CSS 变量消费，移动端统一 0.88 派生。PC 大字号二级分类可通过滚轮/横向滚动条访问完整列表；进入高级与视觉后背景、尺寸、卡片表面和分类视觉控件直接可见，不再通过折叠区隐藏；定向测试和 type-check 通过。

**菜单位置（当前实现）**：放入现有设置面板“高级与视觉”二级菜单；该组控制背景、尺寸、卡片表面及分类标题/图标视觉，进入菜单后直接展示全部控件。

**原始菜单位置决策**：此前曾放入“外观与卡片”标签页的“高级设置”折叠区；该方案因高级与视觉拆为独立菜单而废止。
